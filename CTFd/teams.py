...
def listing():
    ...
    # else:
    count = Teams.query.filter_by(banned=False, Teams.region != 'root').count()
    teams = Teams.query.filter_by(banned=False, Teams.region != 'root').slice(page_start, page_end).all()
